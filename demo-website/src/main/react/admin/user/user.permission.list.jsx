...
export default class UserPermissionList extends React.Component {
    constructor(props) {
        super(props);
                        ...
                        return (
                            <span className="el-table__actions">
                                <Button type="text"> <Link
                                    to={{pathname: "/admin/user/permission/" + data.id + "/view"}}> {i18n.t("user.view")} </Link></Button>
                                <ElementUI.PermissionRequired permissions={["user.write"]}>
                                    <Button type="text"> <Link
                                        to={{pathname: "/admin/user/permission/" + data.id + "/update"}}>{i18n.t("user.update")}</Link> </Button>
    ...
    find() {
        fetch("/admin/api/user/permission/find", {
            method: "PUT",
    ...
    delete(data) {
        MessageBox.confirm(i18n.t("user.permissionDeleteContent"), i18n.t("user.delete"), {type: "warning"}).then(() => {
            fetch("/admin/api/user/permission/batch-delete", {
                method: "POST",
        ...
        return (
            <div className="page">
                    ...
                    <div className="toolbar-buttons">
                        <ElementUI.PermissionRequired permissions={["user.write"]}>
                            ...
                            <Button type="primary"><Link
                                to={{pathname: "/admin/user/permission/create"}}>{i18n.t("user.create")}</Link></Button>
                        </ElementUI.PermissionRequired>
                    </div>
                </div>
                ...
                <div className="footer">
                    <Pagination layout="total,sizes,prev,pager,next,jumper" total={this.state.data.total} pageSizes={this.state.limitOptions} pageSize={this.state.query.limit} currentPage={this.state.query.page} onSizeChange={limit => this.queryChange("limit", limit)} onCurrentChange={page => this.queryChange("page", page)}/>
                </div>
            </div>
        );
    }
